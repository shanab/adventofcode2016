...
func isSSLIP(ip string) bool {
	// TODO
	withinBrackets := false
	abasOutsideBrackets := make([]*Aba, 0)
	abasInsideBrackets := make([]*Aba, 0)
	...
	for i := 0; i < len(ip)-2; i++ {
		...
		default:
			if ip[i] == ip[i+2] && ip[i] != ip[i+1] {
				if withinBrackets {
					abasInsideBrackets = append(abasInsideBrackets, NewAba(ip[i], ip[i+1]))
				} else {
					abasOutsideBrackets = append(abasOutsideBrackets, NewAba(ip[i], ip[i+1]))
				}
			}
		}
	}

	for _, aba := range abasOutsideBrackets {
		for _, bab := range abasInsideBrackets {
			if aba.a == bab.b && aba.b == bab.a {
				return true
			}
		}
	}
