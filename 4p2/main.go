...
func NewOccurence(char byte) *Occurence {
	return &Occurence{
		char:       char,
		occurences: 1,
	}
...
type Occurences struct {
	occurencesMap  map[byte]*Occurence
	occurencesList []*Occurence
}

func NewOccurences() *Occurences {
	return &Occurences{
		occurencesMap:  make(map[byte]*Occurence),
		occurencesList: make([]*Occurence, 0),
	}
}

func (o *Occurences) Add(char byte) {
	occurence, ok := o.occurencesMap[char]
	if !ok {
		occurence = NewOccurence(char)
		o.occurencesMap[char] = occurence
		o.occurencesList = append(o.occurencesList, occurence)
		return
	}

	o.occurencesMap[char].occurences++
}

func (o *Occurences) Len() int {
	return len(o.occurencesList)
}

func (o *Occurences) Swap(i, j int) {
	o.occurencesList[i], o.occurencesList[j] = o.occurencesList[j], o.occurencesList[i]
}

func (o *Occurences) Less(i, j int) bool {
	if o.occurencesList[i].occurences == o.occurencesList[j].occurences {
		return o.occurencesList[i].char < o.occurencesList[j].char
	}

	return o.occurencesList[i].occurences > o.occurencesList[j].occurences
}

func main() {
	fmt.Println("Sum of Sectors:", SumOfSectors())
}

func SumOfSectors() int {
	var result int

	rooms := strings.Split(INPUT, "\n")
	for _, room := range rooms {

		result += roomChecksum(room)
	}
	return result
}

func roomChecksum(room string) int {
	checksum := room[len(room)-6 : len(room)-1]
	roomWithoutChecksum := room[:len(room)-7]
	sep := strings.LastIndex(roomWithoutChecksum, "-")
	normalizedRoom := roomWithoutChecksum[:sep]
	sectorIdStr := roomWithoutChecksum[sep+1:]
	sectorId, _ := strconv.Atoi(sectorIdStr)

	if validRoom(normalizedRoom, checksum) {
		decrypted := decryptRoom(normalizedRoom, sectorId)
...
func validRoom(room, checksum string) bool {
	occurences := NewOccurences()
	for i := 0; i < len(room); i++ {
		if room[i] == '-' {
			continue
		}
		occurences.Add(room[i])
	}

	sort.Sort(occurences)
	sum := make([]byte, 0)
	for _, occurence := range occurences.occurencesList {
		sum = append(sum, occurence.char)
	}

	return checksum == string(sum)[:5]
}

func decryptRoom(room string, sectorId int) string {
	shifts := byte(sectorId % 26)
	roomBytes := []byte(room)
	for i := 0; i < len(roomBytes); i++ {
		if roomBytes[i] == '-' {
			roomBytes[i] = ' '
		} else {
			roomBytes[i] += shifts
			if roomBytes[i] > 'z' {
				roomBytes[i] = 'a' + (roomBytes[i] - 'z' - 1)
			}
		}
	}

	return string(roomBytes)
}
