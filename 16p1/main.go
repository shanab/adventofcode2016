...
func main() {
	fmt.Println(FindChecksum(INPUT, LENGTH))
}

func FindChecksum(s string, length int) string {
	data := []byte(s)

	for len(data) < length {
		next := reverseAndInvert(data)
		data = append(data, '0')
...
func reverseAndInvert(input []byte) []byte {
	result := make([]byte, len(input))
	for i := 0; i < len(input); i++ {
		if input[len(input)-i-1] == '0' {
			result[i] = '1'
		} else {
			result[i] = '0'
		}
